...
CanvasCard.prototype.measure=function(viewmode,width,height)
{
	this.fontsize=30;
	...
	if(viewmode)	this.lineWidth=c.measureText(this.grid[0].join("")).width;//+100;
	else this.lineWidth=c.measureText(this.grid[0].join("")+"     ").width;//+100;
 ...
 */
CanvasCard.prototype.drawValuesNew=function(highContrast)
{
	var colorArray;
	...
	for(var j=1; j<this.grid[0].length;j++){
		////add space character
		if(j%6==0) this.grid[0].splice(j+(spacecounter++), 0, " ");		
	}			
	c.fillText(this.grid[0].join(""),x+codegap,y);
		...
		for(var j=1; j<this.grid[i].length;j++){
			if(j%6==0) {				
				...
				c.fillText( 
					...
					//correct x position for placing code number
					x+(j-6+spacecounter)*(this.charWidth)-codegap/2.5,
					y+(this.codefontsize/2)
				);
				//add space character
				//this.grid[i].splice(j+spacecounter, 0, " ");		
				if(j < 30) this.grid[i].splice(j+spacecounter, 0, " ");		
				
				counter++;
				spacecounter++;//and count how much spaces there are
				console.log("counter:"+counter+"; j:"+j+"; length:"+(this.grid[i].length)+"; j%6:"+(j%6));
			}
			
		}
		
		c.font=this.fontsize+"px "+this.font;
		c.fillText(this.grid[i].join(delimiter),x+codegap,y);
		console.log(this.grid[i]);
		//c.font=this.codefontsize+"px "+this.codefont;
		//c.fillText(counter.toString(),x,y+(this.codefontsize/2));
		
	}
	y+=this.fontsize*1.5;
	c.font=this.codefontsize+"px "+this.codefont;
	var seedsize=c.measureText(this.seed).width;
	var overridefooter = document.getElementById("overrivefooter").checked;
    if(overridefooter){
		c.fillText(document.getElementById("overriveseedvalue").value,(c.canvas.width/2)-(seedsize/2),y);
	} else
